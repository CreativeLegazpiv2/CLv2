...
export default function RegisterForm() {
    ...
    const handleOptionToggle = (option: string) => {
        setAnswers(prevAnswers => {
            const newCreativeField = prevAnswers.creativeField.includes(option)
                ? prevAnswers.creativeField.filter(item => item !== option)
                : [...prevAnswers.creativeField, option];
            return { ...prevAnswers, creativeField: newCreativeField };
        });
        setError(null);
    };
    ...
    const submitForm = async () => {
        ...
        formData.append('entry.1423761216', getValueOrNone(answers.firstName)[0]);
        formData.append('entry.596935029', getValueOrNone(answers.address)[0]);
        formData.append('entry.1486545489', getValueOrNone(answers.mobileNumber)[0]);
        formData.append('entry.905488960', getValueOrNone(answers.email)[0]);
        formData.append('entry.266776651', getValueOrNone(answers.bio)[0]);
        formData.append('entry.933937910', getValueOrNone(answers.instagram)[0]);
        formData.append('entry.2052783211', getValueOrNone(answers.facebook)[0]);
        formData.append('entry.1476922231', getValueOrNone(answers.twitter)[0]);
        formData.append('entry.133240640', getValueOrNone(answers.portfoliolink)[0]);
    
        // Append each selected option for creativeField as a separate entry
        const creativeFields = getValueOrNone(answers.creativeField);
        creativeFields.forEach((field) => {
            formData.append('entry.873872541', field);
        });
    
        try {
            const response = await fetch('https://docs.google.com/forms/d/e/1FAIpQLScSyKe6QAmaAvveOoKVXWaz3uGvdy_UglcU4wAYizFgQa9jhw/formResponse', {
                method: 'POST',
                body: formData,
                mode: 'no-cors'
            });
